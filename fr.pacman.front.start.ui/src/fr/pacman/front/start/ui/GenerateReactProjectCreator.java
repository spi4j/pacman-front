 ...
 */
public class GenerateReactProjectCreator {

	/**
	 * Chemin par défaut du template ZIP React dans le plugin
	 */

	//private static final String c_default_template_path = "/resources/templates/react-ts-vite-template.zip";
	
	private static final String c_default_dsfr_spa_template_path = "/resources/templates/react-ts-vite-dsfr-spa-template.zip";

	/**
	 * Crée un projet Eclipse avec le template React/Vite (sous forme de fichier
	 * zip).
	 * 
	 ...
	 */
	public static void createProject(IProject p_project, IProgressMonitor p_monitor) throws CoreException {
		if (!p_project.exists()) {
		...
		try (InputStream zipStream = findTemplateZip(c_default_dsfr_spa_template_path)) {
			Path targetDir = p_project.getLocation().toFile().toPath();
	...
	private static InputStream findTemplateZip(String p_templatePath) throws Exception {
		InputStream zipStream = GenerateReactProjectCreator.class.getResourceAsStream(p_templatePath);
		...
		System.out.println(
				"Classpath : " + GenerateReactProjectCreator.class.getProtectionDomain().getCodeSource().getLocation());

		// Fallback pour plugin packagé
		Bundle bundle = FrameworkUtil.getBundle(GenerateReactProjectCreator.class);
		if (bundle != null) {
